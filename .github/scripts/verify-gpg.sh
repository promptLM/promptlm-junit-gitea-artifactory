...
if [ -z "${MAVEN_GPG_PRIVATE_KEY:-}" ]; then
  echo "MAVEN_GPG_PRIVATE_KEY is empty."
  exit 1
fi
...
if ! printf '%s\n' "$MAVEN_GPG_PRIVATE_KEY" | head -n1 | grep -q "BEGIN PGP PRIVATE KEY BLOCK"; then
  echo "MAVEN_GPG_PRIVATE_KEY does not look like an ASCII-armored key."
  exit 1
fi
